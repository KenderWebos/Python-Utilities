...
async def dolar(ctx):
    ...
    mainData = Mindicador("dolar").InfoApi()
    await ctx.send( mainData.get("serie")[0].get("valor") )
...
@bot.command()
async def sayTo(ctx, member : discord.Member, *, arg):
    await ctx.channel.purge(limit=1)
    #await ctx.send(str(arg))
    await member.send(str(arg))
...
@bot.command()
async def Iniciando(ctx):
    await ctx.send('*Se despierta*')
...
async def on_ready():
    ...
    url="https://www.twitch.tv/kenderwebos"))
    print("*Se despierta*")
    print("4CT1V4T3D...")
